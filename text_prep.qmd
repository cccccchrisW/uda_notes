...
1. Normalize: convert words mean the same thing to a common place
...
```
######   it normalize things but does not retain the word meaning all the time, make words into smaller form
...
#### lemma is: take common root form of a natural word
#### if we can lemma why steam? lemma is faster
...
```
# Stop words is like link words/conjuntions those ones don't affect your understanding
# this help you to reduce the words that doesn't matter(noise in data) to help you understand the pattern more
...
one way of representing word as....
...
```
tfidf---document term metrics
very column should be unique, individal real word, if not, you missed cleaning the data somewhere, copperhead metrics: the importance of that certain words for this song; HFL:
